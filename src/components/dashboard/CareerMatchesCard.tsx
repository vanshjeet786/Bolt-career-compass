...
export const CareerMatchesCard: React.FC<CareerMatchesCardProps> = ({ assessments, globalMode }) => {
  ...
  // We no longer need showAll since the user requested a strict limit of 5
  ...
  useEffect(() => {
    // Only take the top 5 matches
    const data = calculateCareerMatches(assessments, mode).slice(0, 5);
    setMatches(data);
  }, [assessments, mode]);
    ...
    <Card className="bg-white border-pink-100 h-full flex flex-col">
      ...
      <div className="flex-1">
         <div className="flex items-baseline space-x-2 mb-4">
          ...
          <span className="text-sm text-gray-500">top matches</span>
        </div>
